...
implements IDocumentLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// Updated by Mukesh 20180912
		if(newRecord && is_ValueChanged(MInOutLine.COLUMNNAME_M_Product_ID))
		{
			System.out.println(getM_InOut_ID());
			MInOut mr=new MInOut(getCtx(), getM_InOut_ID(), get_TrxName());
			MDocType doc=new MDocType(getCtx(), mr.getC_DocType_ID(), get_TrxName());
			if(!mr.isSOTrx() && doc.getDocBaseType().equalsIgnoreCase(MDocType.DOCBASETYPE_MaterialReceipt) &&
					mr.getC_Order_ID()<=0)
			{
				log.saveError("Error", "PO is not selected at MR window");
				return false;
			}
		}
	...
	//		;
		...
		/**
		 * Applied check by Mukesh @20180616.... Excel point no : 12
		 */
		/// Start code
		String checkCondition=org.businessView.utils.UtilShipment.checkReservedAndMovementQty(this,this.getC_OrderLine_ID());
		if(checkCondition!=null && checkCondition.length()>0)
		{
			log.saveError("Error", checkCondition);
			return false;
		}
		/// end of Code 
