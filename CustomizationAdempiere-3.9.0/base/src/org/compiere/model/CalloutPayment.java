...
public class CalloutPayment extends CalloutEngine
{
	...
	public String taxAmounts(Properties ctx, int WindowNo, GridTab mTab,
			GridField mField, Object value, Object oldValue)
		{
			...
			if(colName.equalsIgnoreCase("C_Tax_ID") || colName.equalsIgnoreCase("PayAmt"))
			{	
	 ...
	 * @return
	 */
	public String taxAmountsInPDC(Properties ctx, int WindowNo, GridTab mTab,
			GridField mField, Object value, Object oldValue)
		{
			if (isCalloutActive ()) // assuming it is resetting value
				return "";
			...
			{	
				...
				int C_Tax_ID = Env.getContextAsInt (ctx, WindowNo, "C_Tax_ID");
				org.compiere.model.MTax mTax=new MTax(ctx, C_Tax_ID, null);
				Object pdcPay=mTab.getValue ("PayAmt");
				if(pdcPay!=null)
				PayAmt = (BigDecimal)pdcPay;
				BigDecimal TaxRate =mTax.getRate();// (BigDecimal)mTab.getValue ("TaxRate");
				log.info("TaxRate : "+TaxRate);
				BigDecimal actualtaxAmount= org.businessView.utils.UtilPayment.getActualTaxAmountAccordingToRate(PayAmt,TaxRate);
				mTab.setValue("TaxAmount", actualtaxAmount);
	 ...
	 * @return
	 */
	public String taxAmountsInAllocate(Properties ctx, int WindowNo, GridTab mTab,
			GridField mField, Object value, Object oldValue)
		{
			if (isCalloutActive ()) // assuming it is resetting value OverUnderAmt
				return "";
			String colName = mField.getColumnName ();
			log.info("colName : "+colName);
			if(colName.equalsIgnoreCase("C_Tax_ID") || colName.equalsIgnoreCase("Amount") 
			   || colName.equalsIgnoreCase("WriteOffAmt") || colName.equalsIgnoreCase("OverUnderAmt") || colName.equalsIgnoreCase("DiscountAmt"))
			{	
				BigDecimal PayAmt=Env.ZERO;
				int C_Tax_ID = Env.getContextAsInt (ctx, WindowNo, "C_Tax_ID");
				org.compiere.model.MTax mTax=new MTax(ctx, C_Tax_ID, null);
				Object pay=mTab.getValue ("Amount");
				if(pay!=null)
				PayAmt = (BigDecimal)pay;
				BigDecimal TaxRate =mTax.getRate();// (BigDecimal)mTab.getValue ("TaxRate");
				log.info("TaxRate : "+TaxRate);
				BigDecimal actualtaxAmount= org.businessView.utils.UtilPayment.getActualTaxAmountAccordingToRate(PayAmt,TaxRate);
				mTab.setValue("TaxAmt", actualtaxAmount);
			}
			return "";
		}
