...
public class MStorage extends X_M_Storage
{
	...
	public static boolean add (Properties ctx, int M_Warehouse_ID, int M_Locator_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID, int reservationAttributeSetInstance_ID,
		BigDecimal diffQtyOnHand, 
		BigDecimal diffQtyReserved, BigDecimal diffQtyOrdered, BigDecimal diffQtyEntered, String trxName)
	{
		MStorage storage = null;
		StringBuffer diffText = new StringBuffer("(");
		...
		/// Added by Mukesh @20180805 To check is Positive Stock entered in Stock
		if(M_Warehouse_ID>0)
		{
			MWarehouse wh = MWarehouse.get(ctx, M_Warehouse_ID);
			if(wh.isPositiveStock())
			{
				BigDecimal qtyAvailable=MStorage.getQtyAvailable(M_Warehouse_ID, M_Product_ID, M_AttributeSetInstance_ID, trxName);
				System.out.println("Available Qty= "+qtyAvailable+" Qty Entered "+diffQtyEntered);
				s_log.fine("Available Qty= "+qtyAvailable+" Qty Entered "+diffQtyEntered);
				if(qtyAvailable.add(diffQtyEntered).compareTo(Env.ZERO)<0)
				{	
					s_log.severe ("No Storage found - M_Locator_ID=" + M_Locator_ID 
							+ ",M_Product_ID=" + M_Product_ID + ",ASI=" + M_AttributeSetInstance_ID);
						return false;
