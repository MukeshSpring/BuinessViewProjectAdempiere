	...
	implements DocAction, ProcessCall
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// @Mukesh- PostPayment Tender Type Checks... @20180914
		if(getTenderType().equalsIgnoreCase(MPayment.TENDERTYPE_Check))
		{
			...
			{	
				log.saveError("Error", "Can not entered postdated check Date");
				return false;
			}
		}
		
		// @Mukesh- Check duplicate check no for same BankAccount no and AP Payment... @20180914
		if(getTenderType().equalsIgnoreCase(MPayment.TENDERTYPE_Check) && is_ValueChanged(MPayment.COLUMNNAME_CheckNo))
		{
			MDocType docTyp=new MDocType(getCtx(), getC_DocType_ID(), get_TrxName());
			if(docTyp.getDocBaseType().equalsIgnoreCase(MDocType.DOCBASETYPE_APPayment))
			{
				System.out.println(getCheckNo());
				MQuery whereClause=new MQuery(MPayment.Table_Name);
				whereClause.addRestriction(MPayment.COLUMNNAME_AD_Client_ID, MQuery.EQUAL, getAD_Client_ID());
				whereClause.addRestriction(MPayment.COLUMNNAME_TenderType, MQuery.EQUAL, MPayment.TENDERTYPE_Check);
				whereClause.addRestriction(MPayment.COLUMNNAME_C_BankAccount_ID, MQuery.EQUAL, getC_BankAccount_ID());
				whereClause.addRestriction(MPayment.COLUMNNAME_CheckNo, MQuery.LIKE, getCheckNo());
				whereClause.addRestriction(MPayment.COLUMNNAME_IsReceipt, MQuery.EQUAL, false);
				Query listOfProdCat =new Query(getCtx(), MPayment.Table_Name, whereClause.getWhereClause(), get_TrxName());
				List<MPayment> payments=listOfProdCat.list();
				if(payments!=null && payments.size()>0)
				{
					System.out.println(payments.size());
					MBankAccount bankAcc=new MBankAccount(getCtx(), getC_BankAccount_ID(), get_TrxName());
					log.saveError("Error", "Check No '"+getCheckNo()+"' is already exist for this BankAccount No i.e "+bankAcc.getAccountNo());
					return false;
				}
			}			
		}
		
		System.out.println(" getC_BankAccount_ID() :  "+getC_BankAccount_ID());
		// @Trifon - CashPayments
